...
- **`ui/window.rs`** — All UI logic (~2500 lines). Manages `ZapWindow` which contains a tabbed notebook of clusters, each with list and calendar views. Handles vim-style modal input (Normal/Insert/Edit/Command/CalendarInsert modes), key event dispatching, and GTK widget construction
